...
// 模式下仍會在 content 前面輸出 reasoning 區塊(think 標記或純文字分析),
...
export async function askMessages(
  ...
  // thinking(MiniMax 在 content 前面加 reasoning 文字,例如開頭 '<' 的 think 區塊
  // 或純文字分析),導致 JSON.parse 在 content 開頭就失敗。
  // 對 MiniMax:
  ...
  while (attempt < MAX_ATTEMPTS) {
    ...
    try {
      ...
      // 例如開頭是 think 區塊或純文字分析(非 '{'),導致 consumer 做 JSON.parse(content) 直接失敗。
